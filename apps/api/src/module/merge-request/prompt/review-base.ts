...
const SEVERITY_LEVELS = [
    ...
    "  These MUST be fixed immediately. This level should be rare — reserve it for issues that are both urgent and clearly catastrophic.",
    "",
    "🟠 High",
    "  Merge-blocking issues. Realistic production incorrect behavior, non-atomic writes that can leave persisted state wrong, broken public contracts, authorization gaps, or serious reliability risks.",
    "  These must be fixed before merging.",
    "",
    "🟡 Mid",
    "  Real issues that should be fixed before merge when practical. Realistic edge cases, performance problems, weak error handling, or non-critical reliability issues.",
    "  Strongly recommended to fix before merging.",
    "",
    "⚪ Low",
    "  Minor readability, style, naming, or small local clarity issues. Keep these rare — only mention them if they genuinely hinder readability.",
    "",
    "🔵 Suggestion",
    "  Optional improvements that are not bugs. Clearer UX, maintainability improvements, refactoring opportunities, or better structure.",
...
const INLINE_COMMENT_FORMAT = [
    ...
    "```",
    "",
    "Severity emojis: 🔴 Critical, 🟠 High, 🟡 Mid, ⚪ Low, 🔵 Suggestion",
    "",
...
const RULES = [
    ...
    "- Critical/High/Mid findings require strong evidence and clear impact. You must be able to point to the specific code path that causes the problem.",
    "- Critical should be rare. Reserve it for issues that are both urgent and clearly catastrophic in production.",
    "- Low/Suggestion should be rare — only when the improvement is clear, local, and unambiguously worth the developer's attention.",
    ...
    "- Classify by production impact, not by how hard the fix is. Do not downgrade severity because the code has a TODO or the author seems aware of the issue.",
].join("\n");
