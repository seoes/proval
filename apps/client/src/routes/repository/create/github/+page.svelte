<script lang="ts">
    ...
    const filteredRepoList = $derived(
        repoSearch.trim()
            ? repoList.filter((r) => r.fullName.toLowerCase().includes(repoSearch.trim().toLowerCase()))
            : repoList,
    );
    ...
    onMount(async () => {
        // Check if app exists
        if (!data.app) {
            await openAlert("GitHub App is not registered. Please register it in Settings > Integration.");
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (installationList.length === 0) {
                    await openAlert("Installation is not found. Please add it in Settings > Integration.");
                    goto("/settings/integration");
                    return;
                }
            }
    ...
    async function continueToRepositories() {
        ...
        isFetching = true;
        try {
            const res = await fetchApi(`/github/app/${appId}/installation/${selectedInstallationId}/repository`);
            if (!res.ok) {
        ...
        {:else if step === 1}
            <Card title="Step 1: Installation">
                <div class="space-y-4">
                            ...
                            <div class="mt-3 flex flex-wrap gap-2" id={_id} role="group">
                                {#each installationList as installation}
                                    <button
                                        ...
                                        onclick={() => (selectedInstallationId = installation.id.toString())}>
                    ...
                    <Button primary onclick={continueToRepositories} disabled={!selectedInstallationId || isFetching}>
        ...
        {#if step === 2}
            <Card title="Step 2: Repository">
                {#if repoList.length === 0}
                    <p class="text-neutral-500">No repositories accessible for this installation.</p>
                {:else}
                    <div class="space-y-4">
                            ...
                            {#snippet children({ id })}
                                <select
                                    ...
                                    {#each filteredRepoList as repo}
                                        <option value={repo.fullName} disabled={repo.alreadyConnected}>
                                            {repo.fullName}{repo.private ? " (private)" : ""}{repo.alreadyConnected
                                                ? " — already connected"
                                                : ""}
                                        </option>
                                    {/each}
                ...
                <Card>
                    <div class="space-y-4">
                        <FormField
                            ...
                            linkLabelToControl={false}>
                            {#snippet children({ id })}
                                <p {id} class="mt-1 font-medium text-neutral-700 dark:text-neutral-300">
                                    {selectedRepoPath}
                                </p>
                            {/snippet}
                        </FormField>
